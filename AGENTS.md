...
формата — в директории `spec/`. Корень — только сервисное: общая мета, `install.sh`,
README, ассеты — и наборы (напр. `sdd/`, `task/`, `mindmap/`);
`spec/` — справочник, не набор.
...
3. Агенты, которые НЕ читают `~/.agents/skills/`, — в их глобальную директорию из таблицы
   ...
   Пример: `ln -s ~/.agents/skills/mindmap-sync ~/.claude/skills/mindmap-sync`
